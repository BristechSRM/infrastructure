...
let getBasicActionsAsync trelloCred id : Async<BasicAction []> = 
    Download.from <| sprintf "https://api.trello.com/1/cards/%s/actions?filter=commentCard&key=%s&token=%s" id trelloCred.Key trelloCred.Token
...
let getCardCommentActionsAsync trelloCred (rawCard : RawTrelloCard) = 
    async { 
        ...
        return { Card = rawCard
                 Actions = basicCommentActions }
    }

let getActionsPerCardAsync trelloCred (rawCards : RawTrelloCard []) = 
    async { 
        return! rawCards
                |> Array.map (getCardCommentActionsAsync trelloCred)
                |> Async.Parallel
    }
